module.exports = [
    {
      kind: "CARD",
      provider_id: "security-advisor",
      id: "mycard",
      short_description: "card short description",
      long_description: "card long description",
      reported_by: {
        id: "myprovider",
        title: "My provider",
        url: "https://example.com"
      },
      card: {
        section: "Business Partner Tools",
        title: "SampleCard",
        subtitle: "Sample Card",
        finding_note_names: [
          `${process.env.account_id}/providers/security-advisor/notes/mynote`
        ],
        elements: [
          {
            kind: "NUMERIC",
            text: "My sample KPI",
            value_type: {
              kind: "KPI",
              kpi_note_name: `${
                process.env.account_id
              }/providers/security-advisor/notes/sample-kpi`
            }
          }
        ],
        badge_text: "Badge text",
        badge_image:
          "data:image/svg+xml;base64,"
      }
    },
  
    {
      kind: "FINDING",
      provider_id: "security-advisor",
      id: "mynote",
      short_description: "Notes short description",
      long_description: "Notes long description",
      reported_by: {
        id: "My provider",
        title: "My provider",
        url: "https://www.example.com"
      },
      finding: {
        severity: "HIGH",
        next_steps: [
          {
            title: "Fix this."
          },
          {
            title: "Fix that."
          }
        ]
      }
    },
    {
      kind: "KPI",
      provider_id: "security-advisor",
      id: "sample-kpi",
      short_description: "kpi short description",
      long_description: "kpi long description",
      reported_by: {
        id: "My provider",
        title: "My provider",
        url: "https://www.example.com"
      },
      kpi: {
        aggregation_type: "SUM",
        severity: "HIGH"
      }
    }
  ];